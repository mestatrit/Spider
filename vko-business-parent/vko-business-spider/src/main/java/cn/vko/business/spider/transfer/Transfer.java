...
public class Transfer {
	...
	public static String transfer(String s) {
		...
		List<Element> supes = e.getElementsByTag("sup");
		if (!Util.isEmpty(supes)) {
			for (Element element : supes) {
				element.replaceWith(new TextNode(StringUtil.format(Latex.sup,
						element.text()), ""));
			}
		}
		List<Element> subes = e.getElementsByTag("sub");
		if (!Util.isEmpty(subes)) {
			for (Element element : subes) {
				element.replaceWith(new TextNode(StringUtil.format(Latex.sub,
						element.text()), ""));
			}
		}
		List<Element> needChange = e.select(".MathJye");
		// List<Element> needChange = e.select(".MathXxb");
	...
	private static String tr1(Element el) {
		...
		if (td1.children().size() == 1
			...
			if (Util.eq("-1", hassize)) {
				return StringUtil.format(Latex.int_, getInt_Para(tr));
			}
		}
		if (tr.children().size() == 2) {
			Element td2 = tr.child(1);
			Element div2 = td2.child(0);
			String hassize = div2.attr("hassize");
			if (Util.eq("-1", hassize)) {
				return td1.text()
						+ StringUtil.format(Latex.int_, getInt_Para(tr));
			}
		}
	...
	private static String tr2(Element el) {
		...
		if (Util.eq(img.attr("src"), Latex.HTML_SUM)) {
			return StringUtil.format(Latex.sum, tr(el.child(0)),
					tr(el.child(1)));
		}
		// TODO C
		return el.text();
