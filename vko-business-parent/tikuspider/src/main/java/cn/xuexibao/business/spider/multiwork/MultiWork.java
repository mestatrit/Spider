...
import cn.xuexibao.business.spider.cookie.ICookieQueue;
import cn.xuexibao.business.spider.pipeline.IMysqlPipeline;
import cn.xuexibao.business.spider.proxy.IProxyQueue;
...
public abstract class MultiWork implements Runnable {
	...
	public void run() {
		...
		while (!Thread.currentThread().isInterrupted()) {
				...
				public void run() {
					...
					try {
						...
						if (s != null) {
							...
							// if (s.getIntStatus() == 3 || s.getIntStatus() ==
							// 5) {
							// status.set(STOP);
							// }
						}
			...
			if (logger.isInfoEnabled()) {
				logger.info("work run  -work status is " + status.get() + " alive is " + threadAlive.get()); //$NON-NLS-1$
			}
