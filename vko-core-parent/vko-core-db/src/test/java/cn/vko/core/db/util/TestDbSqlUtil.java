...
public class TestDbSqlUtil {
	...
	public void testFetchSignleNormal() {
		...
		assertEquals(DbSqlUtil.fetchInt(dbDao,
				"select type from mock_entity where id=" + entity.getId()),
				type);
		...
		assertEquals(DbSqlUtil.fetchString(dbDao,
				"select name from mock_entity where id=" + entity.getId()),
				name);
		...
		assertEquals(DbSqlUtil.fetchLong(dbDao,
				"select id from mock_entity where id=" + entity.getId()),
				entity.getId());
	}

	@Test
	public void testFetchIntIll() {
		assertEquals(-1, DbSqlUtil.fetchInt(dbDao,
				Sqls.create("select type from mock_entity where id =-1")));
	}
	...
	public void testFetchLongIll() {
		assertEquals(
				-1L,
				DbSqlUtil.fetchLong(dbDao,
						Sqls.create("select id from mock_entity where id =-1")));
	}

	@Test
	public void testFetchStringIll() {
		assertTrue(isEmpty(DbSqlUtil.fetchString(dbDao,
				Sqls.create("select id from mock_entity where id =-1"))));
	}
	...
	public void testQueryIntIll() throws Exception {
		int[] result = DbSqlUtil
				.queryInt(
						dbDao,
						Sqls.create("select type from mock_entity_4_extend_query where id=-1"));
		assertTrue(isEmpty(result));
	}

	@Test
	public void testQueryIntAll() throws Exception {
		int[] result = DbSqlUtil.queryInt(dbDao,
				Sqls.create("select type from mock_entity_4_extend_query"));
	...
	public void testQueryIntPagerRecordCount() throws Exception {
		...
		int[] result = DbSqlUtil.queryInt(dbDao,
				Sqls.create("select type from mock_entity_4_extend_query"),
				Sqls.create("select count(*) from mock_entity_4_extend_query"),
				pager);
		assertEquals(result.length, pager.getPageSize());
	...
	public void testQueryIntCastIll() throws Exception {
		DbSqlUtil.queryInt(dbDao,
				Sqls.create("select name from mock_entity_4_extend_query"));
	}

	@Test
	public void testQueryEntity() throws Exception {
		List<MockEntity4ExtendQuery> list = DbSqlUtil.query(dbDao,
				MockEntity4ExtendQuery.class,
	...
	public void testQueryEntityIll() throws Exception {
		List<MockEntity4ExtendQuery> list = DbSqlUtil
				.query(dbDao, MockEntity4ExtendQuery.class, Sqls
						.create("select *,3 from mock_entity_4_extend_query"),
						null);
		assertEquals(list.size(), totalQuerySize);
	}
	...
	public void testQueryEntityPager() throws Exception {
		Pager pager = new Pager().setPageSize(23);
		List<MockEntity4ExtendQuery> result = DbSqlUtil.query(dbDao,
				MockEntity4ExtendQuery.class,
	...
	public void testFetchEntity() throws Exception {
		...
		assertEquals(entity,
				DbSqlUtil.fetchEntity(dbDao, MockEntity.class, sql));
	}
}
