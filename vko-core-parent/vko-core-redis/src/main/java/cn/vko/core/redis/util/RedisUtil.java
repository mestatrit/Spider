...
public final class RedisUtil {
	 ...
	 * @see RedisKeyPrefix
	 */
	public static String key(final RedisKeyPrefix redisKeyPrefix,
		...
		return sb.append(REDIS_KEY_JOIN_SIGN)
				.append(join(REDIS_KEY_JOIN_SIGN, objects)).toString();
	}
	 ...
	 *            唯一key
	 */
	public static boolean exists(final IRedisDao daoIn, final String type,
	 ...
	 *            值
	 */
	public static void set(final IRedisDao daoIn, final String type,
			final String keyValue, final String value) {
		checkDao(daoIn);
		checkEmpty(type, "type不能为空!");
		checkEmpty(keyValue, "keyValue不能为空!");
	 ...
	 * @return 对应值
	 */
	public static String get(final IRedisDao daoIn, final String type,
			final String keyValue) {
		checkDao(daoIn);
		checkEmpty(type, "type不能为空!");
		checkEmpty(keyValue, "keyValue不能为空!");
	 ...
	 * @return 对应值
	 */
	public static long del(final IRedisDao daoIn, final String type,
			final String keyValue) {
		checkDao(daoIn);
		checkEmpty(type, "type不能为空!");
		checkEmpty(keyValue, "keyValue不能为空!");
	 ...
	 */
	public static long expireAt(final IRedisDao daoIn, final String type,
			final String keyValue, final long unixTime) {
		checkDao(daoIn);
		checkEmpty(type, "type不能为空!");
		checkEmpty(keyValue, "keyValue不能为空!");
		return daoIn.expireAt(key(RedisKeyPrefix.UNI, type, keyValue),
				unixTime / 1000);
	}
	 ...
	 * @return 设置成功返回 1 。当 key 不存在或者不能为 key 设置生存时间时，返回 0
	 */
	public static long expire(final IRedisDao daoIn, final String type,
			final String keyValue, final int seconds) {
		checkDao(daoIn);
		checkEmpty(type, "type不能为空!");
		checkEmpty(keyValue, "keyValue不能为空!");
	 ...
	 */
	public static long expireMin(final IRedisDao daoIn, final String type,
			final String keyValue, final int seconds) {
		checkDao(daoIn);
		checkEmpty(type, "type不能为空!");
		checkEmpty(keyValue, "keyValue不能为空!");
	 ...
	 * @return key主键列表
	 */
	public static Set<String> removeKeys(final IRedisDao daoIn,
