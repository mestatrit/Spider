...
public final class RedisCountUtil {
	 ...
	 * @return 增加后值
	 */
	public static long incrBy(final IRedisDao dao, final String key,
			final long incr) {
		checkDao(dao);
	 ...
	 * @return 减少后值
	 */
	public static long decrBy(final IRedisDao dao, final String key,
			final long decr) {
		checkDao(dao);
	...
	/**
	 * key对应值 +1,并会在一定时间后过期 注意过期的和普通的键值前缀不同·！！！
	 * 
	 * @param dao
	 *            dao对象
	 * @param key
	 ...
	 */
	public static long incrBy(final IRedisDao dao, final String key,
			final long incr, final int seconds) {
		checkDao(dao);
		String realKey = key(EXP_COUNT, key);
		long ttl = dao.ttl(realKey);
		if (ttl == -1) {
	 ...
	 */
	public static long decrBy(final IRedisDao dao, final String key,
			final long decr, final int seconds) {
		checkDao(dao);
		String realKey = key(EXP_COUNT, key);
		long ttl = dao.ttl(realKey);
		if (ttl == -1) {
	 ...
	 * @return 增加后的值
	 */
	public static long incrAt(final IRedisDao dao, final String key,
			final long time) {
		checkDao(dao);
		String realKey = key(EXP_COUNT, key);
		long ttl = dao.ttl(realKey);
	 ...
	 * @return 增加后值
	 */
	public static long incrByAt(final IRedisDao dao, final String key,
			final long incr, final long time) {
		checkDao(dao);
		String realKey = key(EXP_COUNT, key);
		long ttl = dao.ttl(realKey);
		if (ttl == -1) {
	 ...
	 * @return 减少后的值
	 */
	public static long decrAt(final IRedisDao dao, final String key,
			final long time) {
		checkDao(dao);
		String realKey = key(EXP_COUNT, key);
		long ttl = dao.ttl(realKey);
	 ...
	 * @return 减少后值
	 */
	public static long decrByAt(final IRedisDao dao, final String key,
			final long decr, final long time) {
		checkDao(dao);
		String realKey = key(EXP_COUNT, key);
		long ttl = dao.ttl(realKey);
		if (ttl == -1) {
	 ...
	 * @return 是否超过最大值
	 */
	public static boolean max(final IRedisDao dao, final String key,
			final int seconds, final long max) {
