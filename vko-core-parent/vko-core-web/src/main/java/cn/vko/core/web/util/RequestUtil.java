...
public class RequestUtil {
	...
	/**
	 * 根据req请求，判断是否需要对str进行jsonp处理， 将处理后的结果返回，并处理响应的头部信息
	 * 
	 * @param req
	 *            请求对象
	 * @param resp
	 ...
	 * @return 转换的后的字符串
	 */
	public static String toJsonP(final HttpServletRequest req,
		...
		String result = StringUtil.format(CALLBACK_FOMART, callback,
				needToJson ? JsonUtil.toJson(str) : str);
	 ...
	 *            内容的类型
	 */
	public static void safeWrite(final HttpServletResponse resp,
	 ...
	 * @return 记录信息的cookie
	 */
	public final static Cookie getCookie(final HttpServletRequest request,
	 ...
	 *            异常信息
	 */
	public static void redirectTo(final HttpServletRequest request,
			final HttpServletResponse resp, final String url, final Throwable th) {
		if (resp == null) {
			return;
		}
		...
		redirectUrl.append(REDIRECT_FROM_KEY).append("=")
				.append(EncodeUtil.urlEncode(oldUrl));
		if (th != null) {
			redirectUrl
					.append("&")
					.append(REDIRECT_ERROR_KEY)
					.append("=")
					.append(EncodeUtil.urlEncode(ExceptionUtil
							.getSimpleMessage(th)));
		}
		try {
