...
public class LinkPageTag extends PageTag {
	...
	protected String getContent() {
		...
		int start = getPageNum() > ne_half ? Math.max(
				Math.min(getPageNum() - ne_half, upper_limit), 0) : 0;
		int stop = getPageNum() > ne_half ? Math.min(getPageNum() + ne_half
				+ (middleNum % 2), np) : Math.min(middleNum, np);
		int begin;
		int end;
	...
	private void getCurrent(final int pageNum, final StringBuilder content) {
		content.append("<span class=\"vkopage_current\">").append(pageNum)
				.append("</span>");
	}
	...
	private void getCurrent(final String showCssName, final String showContent,
			final StringBuilder content) {
		content.append("<span class=\"").append(showCssName);
		content.append(" vkopage_disabled\">").append(showContent)
				.append("</span>");
	}
	 ...
	 *            添加的样式
	 */
	protected void buildLink(final StringBuilder content, final int showNumber,
			final String words, final String css) {
		content.append("<a href=\"").append(getRealLink(showNumber))
				.append("\"");
		if (!Util.isEmpty(css)) {
