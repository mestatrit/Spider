...
public class DateTimeUtil {
	public static final String FOLDER_FORMAT = "yyyy" + File.separator + "MM"
			+ File.separator + "dd" + File.separator;
	...
	public final static boolean sameMinute(final DateTime time) {
		...
		DateTime now = now();
		return now.getMinuteOfHour() == time.getMinuteOfHour()
				&& sameHour(time);
	}
	 ...
	 */
	public final static Date string2Date(final String dateString,
			final String format) {
		DateFormat dateFormat;
		if (isEmpty(format)) {
			dateFormat = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss",
	 ...
	 */
	public final static DateTime string2DateTime(final String dateString,
			final String format) {
		DateFormat dateFormat;
		if (isEmpty(format)) {
			dateFormat = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss",
	 ...
	 * @return 天数
	 */
	public static final long daysOfTwoTime(final DateTime firstTime,
	 ...
	 * 在当前日期后增加一年时间
	 * 
	 * 例如： 今天是2014-01-04, 顺延一年后就是2015-01-04
	 */
	...
	public static String duration2time(final int seconds) {
		...
		sb.append(h <= 0 ? "" : (h + "小时")).append(m <= 0 ? "" : (m + "分钟"))
				.append(s <= 0 ? "" : (s + "秒"));
		return sb.toString();
	}
}