...
public class TestConvertUtil {
	...
	public void testObj2Map() {
		...
		assertEquals(age, obj2long(simpleResult.get("age")));
		assertEquals(list,
				JsonUtil.fromJsonAsList(String.class, result.get("list")));
		assertEquals(map,
				JsonUtil.fromJsonAsMap(String.class, result.get("map")));
		// TODO 时间未测试通过
		assertEquals(new Date(now.getTime()).toString(), new Date(
				entity(result).getRegisterTime().getTime()).toString());
		MockEntity tranEntity = ConvertUtil
				.map2Object(result, MockEntity.class);
		assertEquals(new Date(now.getTime()).toString(), new Date(tranEntity
				.getRegisterTime().getTime()).toString());
	}
